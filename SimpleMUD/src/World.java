...
public class World {
	...
	public void drawMap(int we, int ns) {
		...
		w_limit = n_limit = e_limit = s_limit = MAPSIZE/2; //ugly solution. Cleaner way to get start position?
			for(int latitude = 0; latitude < MAPSIZE; latitude++) {
				for(int longitude = 0; longitude < MAPSIZE; longitude++) {
					if(map[longitude][latitude] != null) {
						if(longitude < w_limit) w_limit = longitude;
						if(longitude > e_limit) e_limit = longitude;
						if(latitude > n_limit) n_limit = latitude;
						if(latitude < s_limit) s_limit = latitude;
					}
				}
			}
		...
		for(int latitude = n_limit; latitude > s_limit; latitude--) {
			for(int longitude = w_limit; longitude < e_limit; longitude++) {
				if(map[longitude][latitude] == null) System.out.print("   ");
				else if(longitude == we && latitude == ns) System.out.print("[P]");
				else if(map[longitude][latitude].existMonster()) System.out.print("[M]");
				else if(map[longitude][latitude].hasItem("Chest")) System.out.print("[C]");
				else if(map[longitude][latitude].notEmpty()) System.out.print("[L]");
				else if(map[longitude][latitude].getVisit()) System.out.print("[_]");
				else if(!(map[longitude][latitude].getVisit()) && map[longitude][latitude].getExists()) System.out.print("[?]");
				else if(!(map[longitude][latitude].getExists())) System.out.print("   ");
			}
			System.out.println("");
		}
	}
	...
	public Item[] spawnItems(int we, int ns, boolean forced) {
		...
		Item[] result = new Item[spawn.size()];
	...
	private int calculateLevel(int we, int ns) {
		int rad_inc = 5;
		int radius = rad_inc;
		int result = 1;
		for(;; result++) {
